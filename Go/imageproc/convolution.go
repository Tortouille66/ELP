...
// // Convolution sans clamp : conserve les valeurs négatives (utile pour Sobel)
// func ConvolveSigned(img Image, kernel [][]int, workers int) [][]int {
// 	out := make([][]int, img.Height)
// 	for i := range out {
// 		out[i] = make([]int, img.Width)
// 	}
...
// Convolution SANS clamp (conserve valeurs négatives), indispensable pour Sobel
func ConvolveSigned(img Image, kernel [][]int, workers int) [][]int {
	out := make([][]int, img.Height)
	for i := range out {
		out[i] = make([]int, img.Width)
